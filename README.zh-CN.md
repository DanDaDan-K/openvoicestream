...
- Pin：fork `integration/v090-sparktts`（tag `1ac0f2b` + patch）、submodule overlay `repin/v090-overlay`、voxedge wheel `0.0.3a0`。v0.9.0 退役 mel 前端（改 WAV-ingest）、新增原生流式 API，并需要绝对路径的 `EDGELLM_PLUGIN_PATH`。详见 [BENCHMARKS.md](BENCHMARKS.md) 和 [`docs/specs/edgellm-v090-tts-re-port.md`](docs/specs/edgellm-v090-tts-re-port.md)。

...
The factory consults server.core.profile_loader.current_profile() for the
...
def _patch_profile(monkeypatch, profile_dict):
    """Patch server.core.profile_loader.current_profile() to return profile_dict."""
...
def _stub_config_builder(monkeypatch, builder_name: str):
    """Stub server.core.voxedge_backend_config.<builder_name> to return a sentinel.
    ...
    """
    from server.core import voxedge_backend_config
    monkeypatch.setattr(
...
def fresh_factory(monkeypatch):
    ...
    from server.core import asr_backend
...
def test_profile_selects_kokoro_trt_tts(monkeypatch):
    ...
    from server.core import tts_backend
